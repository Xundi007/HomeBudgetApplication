...
public class FragmentYear extends Fragment {
	...
	public FragmentYear(List<Transaction> incomeList, List<Transaction> spendingList, FragmentRefresh fragmentRefresh){
	...
	private void load(){
		...
		for (Transaction e : incomeList ) {
			if(new Date(e.getCreatedTime()).after(new Date(new Date(e.getCreatedTime()).getYear())))
